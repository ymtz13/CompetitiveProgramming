...
class Temperature:
  ...
  def zero_likelihood(self, temp: int, inp: Input):
  ...
  def print_fingerprints(self, inp: Input):
    for i_exit, (fingerprint,
                 (y, x)) in enumerate(zip(self.fingerprints, inp.exit_cells)):
      print("i_exit:{:3d} ({:4d},{:4d}): {:015b}".format(
          i_exit, y, x, fingerprint))
...
class Score:
  ...
  def __init__(self, error_count: int, temperature: Temperature,
               measurements: Measurements):
    ...
    self.total_cost = self.temperature_cost + self.measurement_cost + self.CONSTANT_COST
  ...
  def line(self):
    return 'score:{:16d},  error_cnt:{:3d},  temp_cost:{:16d},  meas_cost:{:16d}'.format(
        self.score, self.error_count, self.temperature_cost,
...
def solve(ProblemClass):
  ...
  for i in range(0, 1001, 100):
    print(i, temp.zero_likelihood(i, inp))
...
if True:
  ...
  print('error_count_mean:{}'.format(error_cnt_sum / 50))
