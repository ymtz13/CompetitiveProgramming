    ...
    def exec(self):
        ...
        for i_hole in range(inp.exit_cell_count):
            ...
            for i_neighbor, (dy, dx) in enumerate(NEIGHBORS):
                ...
                likely_fingerprints = next_likely_fingerprints[:2048]

            # if i_hole == 0:
            #     print(likely_fingerprints)
        ...
        for i_hole, estimated_fingerprint in enumerate(estimated_fingerprints):
            for i_exit, fingerprint in enumerate(self.temperature.fingerprints):
                if fingerprint in estimated_fingerprint:
                    priority.append(
                        (-estimated_fingerprint[fingerprint], i_hole, i_exit)
                    )
                else:
                    priority.append((1, i_hole, i_exit))
