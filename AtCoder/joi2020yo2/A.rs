fn main() {
    ...
    let r1mat = rotate(&smat);
    let r2mat = rotate(&r1mat);
    let r3mat = rotate(&r2mat);

    let tmp = vec![
        compare(&tmat, &smat),
        compare(&tmat, &r1mat) + 1,
        compare(&tmat, &r2mat) + 2,
        compare(&tmat, &r3mat) + 1,
    ];
    let ans = tmp.iter().min();

    println!("{:?}", ans.unwrap());
}

fn rotate(mat: &Vec<Vec<char>>) -> Vec<Vec<char>> {
    let n = mat.len();
    let mut rmat = vec![vec![' '; n]; n];
    for i in 0..n {
        for j in 0..n {
...
fn compare(mat1: &Vec<Vec<char>>, mat2: &Vec<Vec<char>>) -> usize {
    let mut cnt = 0;
    for (row1, row2) in mat1.iter().zip(mat2.iter()) {
        for (&elm1, &elm2) in row1.iter().zip(row2.iter()) {
            if elm1 != elm2 {
                cnt += 1;
            }
        }
