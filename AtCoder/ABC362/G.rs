...
fn main() {
    let s: Vec<_> = std::iter::once(b'z' + 1).chain(read_line().bytes()).map(|c| c as usize).collect();
    let q: usize = read_line().parse().unwrap();
    let sa = sa_is(&s);
    let n = s.len() - 1;

    for _ in 0..q {
        let t: Vec<_> = read_line().bytes().map(|c| c as usize).collect();

        let mut l = 0;
        let mut w = 1 << 20;
        while w > 0 {
            let i = l + w;
            if i < sa.len() && &s[sa[i]..=n] < &t {
                l = i;
            }
        ...
        while w > 0 {
            let i = r + w;
            if i < sa.len() && n + 1 - sa[i] >= t.len() && &s[sa[i]..=n][..t.len()] == &t {
                r = i;
            }
...
// ['i', 'ippi', 'isippi', 'issisippi', 'missisippi', 'pi', 'ppi', 'sippi', 'sisippi', 'ssisippi']
// [9, 6, 4, 1, 0, 8, 7, 5, 3, 2]
