...
fn main() {
    ...
    let mut elist = vec![vec![]; n];
    for i in 0..n - 1 {
        let (a,): (usize,) = Read::read();
        for _ in 0..a {
            let (p, q): (usize, usize) = Read::read();
            elist[p - 1].push((q - 1, i));
            elist[q - 1].push((p - 1, i));
        }
    }
    ...
    while let Some((i, d)) = queue.pop_front() {
        ...
        for &(r, j) in &elist[i] {
            if dist[r] == -1 || dist[j] != -1 {
                continue;
            }
            queue.push_back((j, std::cmp::max(d, dist[r]) + 1));
        }
    }
