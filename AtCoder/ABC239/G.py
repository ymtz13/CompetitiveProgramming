...
def max_flow(N: int, S: int, T: int, edges: List[Tuple[int, int, int]]):
    ...
    return total_flow, [t for t in range(N) if visited[t] == trial]
...
for _ in range(M):
    ...
    a -= 1
    b -= 1
    edges.append((a * 2 + 1, b * 2, INF))
    edges.append((b * 2 + 1, a * 2, INF))

C = map(int, input().split())
for i, c in enumerate(C):
    edges.append((i * 2, i * 2 + 1, c if c else INF))

mf, s = max_flow(N * 2, 0, N * 2 - 1, edges)
print(mf)

s = set(s)
ans = []
for f, t, c in edges:
    if f in s and t not in s:
        ans.append(f // 2 + 1)
