...
C = [Cx, Cy]
queue = deque([(0, x1, y1, 1, 0), (1, x1, y1, 1, 0)])
...
while queue:
  ...
  if l == 0:
    if Cx[x + 1][y] is None: queue.append((0, x + 1, y, ss, tt))
    if Cx[x - 1][y] is None: queue.append((0, x - 1, y, ss, tt))
    if Cy[x][y] is None: queue.append((1, x, y, s + 1, 0))

  if l == 1:
    if Cy[x][y + 1] is None: queue.append((1, x, y + 1, ss, tt))
    if Cy[x][y - 1] is None: queue.append((1, x, y - 1, ss, tt))
    if Cx[x][y] is None: queue.append((0, x, y, s + 1, 0))
