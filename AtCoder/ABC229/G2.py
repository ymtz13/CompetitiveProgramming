...
def mergeL(vL, vR):
  nSl, nYl, cl = vL
  nSr, nYr, cr = vR
  return (nSl + nSr, nYl + nYr, cl + cr + nSl * nYr)


def mergeR(vL, vR):
  nSl, nYl, cl = vL
  nSr, nYr, cr = vR
  return (nSl + nSr, nYl + nYr, cl + cr + nSr * nYl)


stL = SegTree(1 << 20, (0, 0, 0), mergeL)
stR = SegTree(1 << 20, (0, 0, 0), mergeR)

for i, c in enumerate(S):
  nS = 1 if c == '.' else 0
  nY = 1 - nS
  stL.update(i, (nS, nY, 0))
  stR.update(i, (nS, nY, 0))
